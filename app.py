...
# --- Page Configuration ---
st.set_page_config(page_title="Titan Vision | AI Tracker", layout="wide", initial_sidebar_state="expanded")

# --- Modern Dashboard CSS ---
st.markdown("""
    <style>
    .stApp { background-color: #0B0E14; color: #E2E8F0; }
    [data-testid="stSidebar"] { background-color: #151A23; border-right: 1px solid #2D3748; }
    h1, h2, h3 { color: #FFFFFF !important; font-family: 'Inter', sans-serif; font-weight: 600; letter-spacing: -0.5px; }
    .highlight { color: #3A86FF; }
    [data-testid="stMetricValue"] { font-size: 2rem !important; color: #3A86FF !important; }
    .stButton>button { background-color: #3A86FF; color: #FFFFFF; border-radius: 6px; border: none; width: 100%; font-weight: 600; transition: all 0.3s; }
    .stButton>button:hover { background-color: #2563EB; box-shadow: 0 4px 12px rgba(58, 134, 255, 0.3); }
    [data-testid="stFileUploadDropzone"] { border: 2px dashed #2D3748; border-radius: 8px; background-color: #1A202C; }
    </style>
...
st.markdown("<p style='color: #A0AEC0; font-size: 1.1rem;'>High-speed computer vision pipeline optimized for real-time object tracking.</p>", unsafe_allow_html=True)
st.divider()
...
selected_classes = st.sidebar.multiselect(
    ...
    default=["Person", "Car", "Motorcycle", "Truck"]
)
